...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"From: {update.message.from_user.username} (Bot: {update.message.from_user.is_bot})")
    logger.info(f"Chat ID: {update.effective_chat.id}")
    logger.info(f"Text: {update.message.text or update.message.caption}")
    ...
    if str(update.effective_chat.id) != SOURCE_CHAT_ID:
        logger.info(f"❌ IGNORED: Not from source chat (expected: {SOURCE_CHAT_ID})")
        return
    ...
    if solana_addresses:
        ...
        logger.info(f"✅ Found {len(solana_addresses)} address(es), {len(unique_addresses)} unique")
        ...
        for idx, address in enumerate(unique_addresses, 1):
            logger.info(f"")
            logger.info(f"========== PROCESSING ADDRESS {idx}/{len(unique_addresses)} ==========")
            logger.info(f"Address: {address}")
            
            try:
                # Send /nar message
                nar_message = f"/nar {address}"
                logger.info(f"📤 STEP 1: Sending /nar message...")
                
                nar_result = await context.bot.send_message(
                    chat_id=DESTINATION_CHAT_ID,
                    text=nar_message
                )
                logger.info(f"✅ STEP 1 COMPLETE: /nar sent successfully (message_id: {nar_result.message_id})")
                
                # Delay
                logger.info(f"⏳ Waiting 1 second...")
                await asyncio.sleep(1)
                
                # Send /soc message
                soc_message = f"/soc {address}"
                logger.info(f"📤 STEP 2: Sending /soc message...")
                
                soc_result = await context.bot.send_message(
                    chat_id=DESTINATION_CHAT_ID,
                    text=soc_message
                )
                logger.info(f"✅ STEP 2 COMPLETE: /soc sent successfully (message_id: {soc_result.message_id})")
                
                logger.info(f"🎉 BOTH MESSAGES SENT FOR: {address}")
                
                # Delay between addresses
                if idx < len(unique_addresses):
                    logger.info(f"⏳ Waiting before next address...")
                    await asyncio.sleep(1)
        ...
        logger.info(f"❌ No Solana addresses found in message")
